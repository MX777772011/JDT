package com.devplatform.${bean.serviceName}.controller;
...
import com.devplatform.${bean.serviceName}.service.${bean.className}Service;

/**
 * ${bean.className} controller
 * 用于${bean.tableName}的接口
 * @author LJY
 *
 */
@Api(value = "${bean.className}Controller", description = "系统自动生成接口", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
@RestController
@RequestMapping("/${bean.lowerName}") 
public class ${bean.className}Controller {
	
	protected final static Logger log= LoggerFactory.getLogger(${bean.className}Controller.class);
	...
	public CommonResult page(${bean.className} model){
	...
	public CommonResult save(${bean.className} bean){
		//后台验证数据
		
		<#list bean.fields as field>
			//TODO ${field.fieldJavaName}后台验证
		</#list>
