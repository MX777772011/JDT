...
public class ZipUtil {
	...
	public static void unzip(String zipFilePath, String unzipFilePath, boolean includeZipFileName) throws Exception {
		...
		if (includeZipFileName) {
			...
			unzipFilePath = unzipFilePath + fileName + File.separator;
		}
		// 创建解压缩文件保存的路径
		File unzipFileDir = new File(unzipFilePath);
		if (unzipFileDir.exists()) {
			FileUtils.deleteFileAndDir(unzipFilePath);
		}
		unzipFileDir.mkdirs();
		// 开始解压
		ZipEntry entry = null;
	...
	private static void zip(String srcRootDir, File file, ZipOutputStream zos) throws Exception {
		...
		if (file.isFile()) {
			...
			if (index != -1) {
				subPath = subPath.substring((srcRootDir.length()-1) + File.separator.length());
			}
	...
	public static void main(String[] args) {
		try {
			unzip("/Users/liyinchu/Documents/wys_workspace/1.zip", "/Users/liyinchu/Documents/wys_workspace/12/", false);
		} catch (Exception e) {
