...
<head>
	<script type="text/javascript">
		function displayChildren(){
			...
			if (!isNull(img) && img.getAttribute('isOpen') != null){
				if (img.getAttribute('isOpen') == 'false'){
		            ...
		            img.setAttribute('src', basePath+'<%=request.getContextPath() %>/images/minus.gif');
				}else{
		            img.setAttribute('isOpen', 'false');
		            img.setAttribute('src', basePath+'<%=request.getContextPath() %>/images/plus.gif');
				}
			}
		    ...
		    if (isToOpen && isByAjax)
		    {
		        ...
		        div.innerHTML = "<img align='absmiddle' border='0' src='"+basePath+"<%=request.getContextPath() %>/images/loading.gif' /> 数据加载中，请稍候...";
				        ...
				        if(e.style.display == '') {
					        ...
					        if (!isNull(imgClickable)){
						        if (!isNull(imgClickable.getAttribute('isOpen')) && imgClickable.getAttribute('isOpen') =='true'){
		                            ...
		                            imgClickable.setAttribute('src', basePath+'<%=request.getContextPath() %>/images/plus.gif');
		...
		function folderSelect(){
			$("#fileFrame").attr("src","showModel.do");
		}
	</script>
</head>
						...
						<div class="widget-content nopadding">						
							<table class="table-condensed table table-bordered table-striped table-hover">
								<tbody>
									<tr>
										<td>
											<img align="absmiddle" style="cursor:pointer" onClick="displayChildren();" isAjax="true" isOpen="false" id="" src="<%=request.getContextPath() %>/images/plus.gif" />						
											<img align="absmiddle" border="0" src="<%=request.getContextPath() %>/images/folder.gif" />&nbsp;
