...
//		this.currentConfirm(msg,option);
	...
	closeTag:function(config){
        ...
        var tabA = $(parentDoc).context.URL.split("#")[1];
	        if(tabA !=null && tabA!=""){
	        	// 2017-01-06 索超超加     为了让页面上所有数据都刷新，所以做了首先关闭了父页面，然后再重新开启
//	        	var currentTab1=$(parentDoc).find("a[data-id='"+config+"']");
	        	if(config != undefined && config != ''){
		        	var currentTab1=$(parentDoc).find("a[data-id='"+config+"']");
		        	var title1 = currentTab1.html();
			        parent.mainMultiTabs.close(currentTab1);
			        config = config.replace(msUrl,"");
		            SPlatForm.createTab(config,{
			     		title : title1
			     	});
	        	}
//	        	$(currentTab1).window.STableUtils.reloadTable();
