...
var STableUtils={
	initBootstrapTable:function(config){//init时，请在config中指定url(必填)和queryparam(选填)
		...
		config.pagination = config.pagination != undefined ? config.pagination : true;// 设置True在表格底部显示分页工具栏。
		...
		config.showHeader = config.showHeader != undefined ? config.showHeader : true;// 显示表头
		config.showColumns =  config.showColumns  != undefined ? config.showColumns : true;// 设置为True可显示表格显示/隐藏列表。
		...
		config.striped =  config.striped != undefined ? config.striped : true;// 使表格带有条纹。
		config.showRefresh = config.showRefresh != undefined ? config.showRefresh : true;// 设置为True可显示刷新按钮。
		config.toolbar = config.toolbar || "#toolbar";// 定义toolbar是哪个
		config.toolbarAlign = config.toolbarAlign || "left";// 对齐方式
		config.clickToSelect = config.clickToSelect != undefined ? config.clickToSelect : true;// 设置为True时点击行即可选中单选/复选框。
		// config.singleSelect = config.clickToSelect != undefined ? config.showRefresh : true;//只能选择一条记录
		// config.checkboxHeader = config.checkboxHeader != undefined ? config.showRefresh : true;//设置为False时隐藏表头中的全选复选框。
		// config.maintainSelected = config.maintainSelected != undefined ? config.showRefresh : true;//设置为True当换页或者搜索时保持选中的行。
		// config.sortable = config.sortable != undefined ? config.showRefresh : true;//设置为False时禁用所有列的排序。
		config.showExport = config.showExport != undefined ? config.showExport : true;// 设置为true，显示导出按钮。
