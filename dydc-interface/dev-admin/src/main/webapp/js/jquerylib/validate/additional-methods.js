var scc_idarray = [7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2];
var check_idarray = [1,0,'X',9,8,7,6,5,4,3,2];
//字母校验
...
jQuery.validator.addMethod("idCard", function(value, element) {
	...
	var re=true;
	re = idCard.test(value);	
	if(re){
		if(value.length!=18)re=false;
		var val = value.split("");
		var  d = 0 ;
		for(var i=0 ; i<17 ;i++){
			d += val[i] * scc_idarray[i];
		}
		var b = d % 11 ; 
		
		if(val[17] != check_idarray[b]){
			re = false;
		}
	}
