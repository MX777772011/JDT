...
<form id="example-advanced-form" action="${msUrl}/sys/createjava/viewTable.do" >
    <h3>数据库连接</h3>
    <fieldset>
    ...
    <h3>生成文件</h3>
    <fieldset>
 	    <input type="checkbox" name="isShwoMenu" id="isShwoMenu"  value="1" /><label for="acceptTerms-2">是否生成菜单</label>
    </fieldset>
</form>
<div class="row" id="create_coloumn">
			<div class="row" v-if="show">
				...
				<div class="col-sm-9">
					<div class="row text-center">限制</div>
					<div class="row">
						<div class="col-sm-1 text-center">非空</div>
						<div class="col-sm-2 text-center">长度/大小</div>
						<div class="col-sm-2 text-center">特殊</div>
						<div class="col-sm-3 text-center">枚举</div>
						<div class="col-sm-3 text-center">正则</div>
					</div>
				</div>
			</div>
			<div class="row" v-for="coloumn in coloumns" v-if="show">
				<div class="col-sm-1 text-center">
					<label class="control-label">{{coloumn.COLUMN_NAME}}</label>
				</div>
				<div class="col-sm-1 text-center">
				...
				<div class="col-sm-9 text-center">
					<div class="row">
						<div class="col-sm-1 text-center">
							<input v-model="coloumn.IS_NULLABLE" type="checkbox" class="form-control text-center" />
						</div>
						<div class="col-sm-2 text-center">
							<form action="" class="form-inline">
								<div class="form-group">
									<input type="text" v-model="coloumn.MIN_LENGTH" class="form-control" style="width: 50px;"/>-<input type="text" v-model="coloumn.MAX_LENGTH" class="form-control"  style="width: 50px;"/>
								</div>
							</form>
						</div>
						<div class="col-sm-2 text-center">
							<label class="checkbox-inline"> <input v-model="coloumn.IS_EMAIL" type="checkbox" class="form-control" />是否邮箱
							</label> <label class="checkbox-inline"> <input v-model="coloumn.IS_MOBILE" type="checkbox" class="form-control" />是否手机号
							</label> <label class="checkbox-inline"> <input v-model="coloumn.IS_IDNO" type="checkbox" class="form-control" />身份证号码
							</label>
						</div>
				...
				if(result.success){
				   ...
				   if(currentIndex == 2){
						...
						for (var i = 0; result.data.length; i++) {
							...
							create_coloumn.coloumns[i].MAX_LENGTH = result.data[i].CHARACTER_MAXIMUM_LENGTH;
						}
				   }
				}else{
